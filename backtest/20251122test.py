...
def create_order_panel(filtered_trades, symbols=None):
    ...
    pnl_col = 'pnl' # 使用处理后的统一PnL列（币计价，保留原始）
    pnl_calc_col = 'pnl_usdt' if 'pnl_usdt' in filtered_trades.columns else pnl_col  # 统计优先用USDT
    ...
    if 'pnl_usdt' in symbol_trades.columns:
        symbol_trades['pnl_usdt'] = symbol_trades['pnl_usdt'].apply(lambda x: safe_to_decimal(x) if x is not None else Decimal(0))
        symbol_trades['pnl_usdt'] = pd.to_numeric(symbol_trades['pnl_usdt'].astype(str), errors='coerce').fillna(0)
    ...
    try:
        ...
        grouped = symbol_trades.groupby([symbol_col, side_col]).agg(
            ...
            总盈亏=(pnl_col, 'sum'),
            总盈亏_USDT=('pnl_usdt', 'sum') if 'pnl_usdt' in symbol_trades.columns else (pnl_calc_col, 'sum'),
    ...
    if st.session_state.processed_data is None: st.warning("未找到处理好的数据。请先前往'数据上传与处理'页面上传并处理数据。")
        ...
        if filtered_data.empty: st.warning("所选条件没有匹配的交易数据")
            ...
            with tab1:
                ...
                try:
                    # --- PnL Statistics ---
                    pnl_calc_col = 'pnl_usdt' if 'pnl_usdt' in filtered_data.columns else pnl_col
                    filtered_data[pnl_calc_col] = filtered_data[pnl_calc_col].apply(safe_to_decimal)
                    total_pnl_coin = filtered_data[pnl_col].sum()
                    total_pnl_usdt = filtered_data[pnl_calc_col].sum()
                    
                    win_trades = filtered_data[filtered_data[pnl_calc_col] > 0]; loss_trades = filtered_data[filtered_data[pnl_calc_col] < 0]
                    ...
                    avg_profit_float = float(win_trades[pnl_calc_col].astype(float).mean()) if win_count > 0 else 0.0
                    avg_loss_float = abs(float(loss_trades[pnl_calc_col].astype(float).mean())) if loss_count > 0 else 0.0
                    ...
                    with col1: 
                        st.metric("总盈亏(原币计)", f"{total_pnl_coin:.4f}")
                    ...
                    with col4: 
                        ...
                        st.metric("费用占利润百分比", f"{(total_fees/total_pnl_usdt*100):.2f}%" if total_pnl_usdt > 0 else "N/A")
                    ...
                    if analysis_type == "按交易对":
                        symbol_pnl = filtered_data.groupby(symbol_col)[pnl_calc_col].agg(['sum', 'count', 'mean']).reset_index()
                        symbol_pnl.columns = [symbol_col, '总盈亏(USDT)', '交易次数', '平均盈亏(USDT)']
                        symbol_pnl = symbol_pnl.sort_values('总盈亏(USDT)', ascending=False)
                        if not symbol_pnl.empty:
                            fig = make_subplots(specs=[[{"secondary_y": True}]])
                            colors = ['green' if pnl >= 0 else 'red' for pnl in symbol_pnl['总盈亏(USDT)']]
                            fig.add_trace(go.Bar(x=symbol_pnl[symbol_col], y=symbol_pnl['总盈亏(USDT)'], marker_color=colors, name="总盈亏(USDT)"), secondary_y=False)
                            ...
                            fig.update_yaxes(title_text="总盈亏(USDT)", secondary_y=False); fig.update_yaxes(title_text="交易次数", secondary_y=True)
                        ...
                        date_pnl = filtered_data.groupby('trade_date')[pnl_calc_col].agg(['sum', 'count']).reset_index()
                        date_pnl.columns = ['日期', '每日盈亏(USDT)', '交易次数']
                        date_pnl['累计盈亏(USDT)'] = date_pnl['每日盈亏(USDT)'].cumsum()
                        if not date_pnl.empty:
                            fig = make_subplots(specs=[[{"secondary_y": True}]])
                            colors = ['green' if pnl >= 0 else 'red' for pnl in date_pnl['每日盈亏(USDT)']]
                            fig.add_trace(go.Bar(x=date_pnl['日期'], y=date_pnl['每日盈亏(USDT)'], marker_color=colors, name="每日盈亏(USDT)"), secondary_y=False)
                            fig.add_trace(go.Scatter(x=date_pnl['日期'], y=date_pnl['累计盈亏(USDT)'], mode='lines', line=dict(color='blue'), name="累计盈亏(USDT)"), secondary_y=False)
                            ...
                            fig.update_yaxes(title_text="盈亏(USDT)", secondary_y=False); fig.update_yaxes(title_text="交易次数", secondary_y=True)
                            ...
                            month_pnl = filtered_data.groupby('year_month')[pnl_calc_col].agg(['sum', 'count', 'mean']).reset_index()
                            month_pnl.columns = ['年月', '月度盈亏(USDT)', '交易次数', '平均盈亏(USDT)']
                            st.dataframe(month_pnl, use_container_width=True)
                        else: st.info("无数据可用于按时间分析。")
                    elif analysis_type == "按交易方向":
                        side_pnl = filtered_data.groupby(side_col)[pnl_calc_col].agg(['sum', 'count', 'mean']).reset_index()
                        side_pnl.columns = ['方向', '总盈亏(USDT)', '交易次数', '平均盈亏(USDT)']
                        if not side_pnl.empty:
                            ...
                            with col2:
                                fig2 = go.Figure()
                                colors = ['green' if pnl >= 0 else 'red' for pnl in side_pnl['总盈亏(USDT)']]
                                fig2.add_trace(go.Bar(x=side_pnl['方向'], y=side_pnl['总盈亏(USDT)'], marker_color=colors, name="总盈亏(USDT)"))
                                fig2.add_trace(go.Scatter(x=side_pnl['方向'], y=side_pnl['平均盈亏(USDT)'], mode='lines+markers', name="平均盈亏(USDT)", line=dict(color='purple')))
                                fig2.update_layout(title="交易方向盈亏分析", xaxis_title="交易方向", yaxis_title="盈亏(USDT)", height=400); st.plotly_chart(fig2, use_container_width=True, key="dashboard_pnl_by_direction")
    ...
    if st.session_state.processed_data is None: st.warning("未找到处理好的数据。请先前往'数据上传与处理'页面上传并处理数据。")
        ...
        if filtered_data.empty: st.warning("所选条件没有匹配的交易数据")
            ...
            try:
                ...
                filtered_data['trade_date'] = pd.to_datetime(filtered_data[time_col]).dt.date
                daily_pnl = filtered_data.groupby('trade_date')[pnl_calc_col].sum().reset_index()
                daily_pnl.columns = ['trade_date', 'daily_pnl_usdt']
                ...
                if not daily_pnl.empty:
                    # Max Drawdown Calculation (USDT)
                    daily_pnl['cumulative_pnl_usdt'] = daily_pnl['daily_pnl_usdt'].cumsum()
                    daily_pnl['running_max'] = daily_pnl['cumulative_pnl_usdt'].cummax()
                    daily_pnl['drawdown'] = daily_pnl['running_max'] - daily_pnl['cumulative_pnl_usdt']
                    max_drawdown = daily_pnl['drawdown'].max()

                    # Sharpe Ratio Calculation (Fixed, USDT)
                    daily_returns_float = daily_pnl['daily_pnl_usdt'].astype(float)
                    ...
                    daily_pnl['is_profit'] = daily_pnl['daily_pnl_usdt'] > 0
                    ...
                    avg_profit = daily_pnl[daily_pnl['daily_pnl_usdt'] > 0]['daily_pnl_usdt'].mean() if win_count > 0 else Decimal(0)
                    avg_loss = abs(daily_pnl[daily_pnl['daily_pnl_usdt'] < 0]['daily_pnl_usdt'].mean()) if loss_count > 0 else Decimal(0)
                    ...
                    with col2: 
                        st.metric("最大回撤(USDT)", f"{max_drawdown:.4f}")
                    ...
                    with col4: 
                        ...
                        st.metric("费用占利润比例", f"{(total_fees/total_pnl_usdt*100):.2f}%" if total_pnl_usdt > 0 else "N/A")

                    # Performance Curve
                    st.markdown('<h3 class="sub-header">策略表现曲线</h3>', unsafe_allow_html=True)
                    fig = make_subplots(rows=2, cols=1, subplot_titles=('每日盈亏和累计盈亏(USDT)', '回撤'), vertical_spacing=0.12, row_heights=[0.7, 0.3])
                    colors = ['green' if pnl >= 0 else 'red' for pnl in daily_pnl['daily_pnl_usdt']]
                    fig.add_trace(go.Bar(x=daily_pnl['trade_date'], y=daily_pnl['daily_pnl_usdt'], marker_color=colors, name="每日盈亏(USDT)"), row=1, col=1)
                    fig.add_trace(go.Scatter(x=daily_pnl['trade_date'], y=daily_pnl['cumulative_pnl_usdt'], mode='lines', name="累计盈亏(USDT)", line=dict(color='blue')), row=1, col=1)
                    ...
                    fig.update_xaxes(title_text='日期', row=1, col=1); fig.update_yaxes(title_text='盈亏(USDT)', row=1, col=1)
                    fig.update_xaxes(title_text='日期', row=2, col=1); fig.update_yaxes(title_text='回撤(USDT)', row=2, col=1)
                    ...
                    if selected_symbols != ["全部"] and len(selected_symbols) > 1:
                        ...
                        # 按交易对和日期计算每日盈亏（USDT口径）
                        symbol_daily_pnl = filtered_data.groupby([symbol_col, 'trade_date'])[pnl_calc_col].sum().reset_index()
                        ...
                        for symbol in selected_symbols:
                            ...
                            if not symbol_data.empty:
                                symbol_data['cumulative_pnl_usdt'] = symbol_data[pnl_calc_col].cumsum()
                        ...
                        if not symbol_cum_pnl.empty:
                            ...
                            for symbol in selected_symbols:
                                ...
                                if not symbol_data.empty:
                                    fig_symbols.add_trace(go.Scatter(
                                        ...
                                        y=symbol_data['cumulative_pnl_usdt'],
                                        mode='lines',
                                        name=f"{symbol} 累计盈亏(USDT)"
                                    ))
                            
                            fig_symbols.update_layout(
                                title="交易对累计盈亏对比(USDT)",
                                xaxis_title="日期",
                            ...
                            symbol_stats = filtered_data.groupby(symbol_col).agg(
                                总盈亏_USDT=(pnl_calc_col, 'sum'),
                                交易次数=(time_col, 'count'),
                                平均盈亏_USDT=(pnl_calc_col, 'mean'),
                                最大盈利_USDT=(pnl_calc_col, lambda x: x.max()),
                                最大亏损_USDT=(pnl_calc_col, lambda x: x.min())
                            ).reset_index()
                            ...
                            for symbol in symbol_stats[symbol_col]:
                                ...
                                wins = len(symbol_trades[symbol_trades[pnl_calc_col] > 0])
                            ...
                            # 按总盈亏排序
                            symbol_stats = symbol_stats.sort_values('总盈亏_USDT', ascending=False)
                    ...
                    if not month_pnl.empty:
                        ...
                        pivot_data = month_pnl.pivot_table(values='月度盈亏(USDT)', index='年', columns='月', aggfunc='sum')
                        if not pivot_data.empty:
                            fig = go.Figure(data=go.Heatmap(z=pivot_data.values, x=pivot_data.columns, y=pivot_data.index, colorscale=[[0, 'red'],[0.5, 'white'],[1, 'green']], colorbar=dict(title='盈亏(USDT)'), hoverongaps=False))
                            fig.update_layout(title='月度盈亏热力图(USDT)', xaxis_title='月份', yaxis_title='年份', height=400)
                            st.plotly_chart(fig, use_container_width=True, key="strategy_monthly_heatmap")
                        st.dataframe(month_pnl[['年月', '月度盈亏(USDT)', '交易次数', '平均盈亏(USDT)']], use_container_width=True)
                    ...
                    try: # Wrap frequency analysis in its own try-except
                        ...
                        if not hour_stats.empty and not weekday_stats.empty:
                            ...
                            fig_pnl_w = go.Figure()
                            colors = ['green' if pnl >= 0 else 'red' for pnl in weekday_stats['总盈亏_USDT']]
                            fig_pnl_w.add_trace(go.Bar(x=weekday_stats['星期'], y=weekday_stats['总盈亏_USDT'], marker_color=colors, name="总盈亏(USDT)"))
                            fig_pnl_w.add_trace(go.Scatter(x=weekday_stats['星期'], y=weekday_stats['平均盈亏_USDT'], mode='lines+markers', name="平均盈亏(USDT)", line=dict(color='purple')))
                            fig_pnl_w.update_layout(title="星期几盈亏分析(USDT)", xaxis_title="星期", yaxis_title="盈亏(USDT)", height=400, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
                            st.plotly_chart(fig_pnl_w, use_container_width=True, key="strategy_weekday_pnl")
                            st.dataframe(weekday_stats[['星期', '总盈亏_USDT', '平均盈亏_USDT', '交易次数']], use_container_width=True)
                        else:
                             st.info("无数据可用于频率分析。")
